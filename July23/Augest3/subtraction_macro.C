void subtraction_macro(double scale_factor, char *histname)
{
TFile *f1 = new TFile("output.root");
TH1F *h1 = (TH1F*)f1->Get(histname);//"gHist");
h1->SetLineColor(kGreen);
h1->Draw();
TFile *f2 = new TFile("MainGoAT_carbon.root");
TH1F *h2 = (TH1F*)f2->Get(histname);//"gHist");
...
TFile f("output_b4subt.root","UPDATE");
histname_h3->Write();
f.Close();
}
